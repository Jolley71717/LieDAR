...
`tools/make_fixture.sh 1` and passes `tools/fixture_audit.sh`; `tools/mutation_check.sh` proves
three tests fail without their code (loop-closure translation, degradation relabel, gating
threshold). The raycaster measures 12 ms per 256 × 192 frame in release on an M4 Pro. The
Example app and the XCUITest journeys (the other half of the phase-2 acceptance row) are a
separate piece of work. `docs/REALISM.md` is written.
