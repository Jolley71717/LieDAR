...
# This script removes one line at a time, runs the test that guards it, and demands that the
# test FAIL on the named assertion — then puts the line back and proves the file is
# byte-for-byte what it was, so a mutation cannot leak into a commit. (Generalised from
# Meshwise's tools/mutation_check_badge.sh.) Cases, each its own mutation and its own test:
#
...
#         "frames written"
#
# Before mutating, the three tests are run once unmodified and must pass, so a red suite is
...
#   PASS  = every case's test failed on its named assertion without its line, and every file
#           was restored byte-identically.
#   FAIL  = a test still passed without its line (it proves nothing), failed elsewhere, did not
#           compile, did not run, or a restore failed.
set -uo pipefail
...
for CASE in "${CASES[@]}"; do
  ...
  grep -vxF "$LINE" "$FILE" > "$FILE.mutated" && mv "$FILE.mutated" "$FILE"
